...
SET TARGET=libinteractive\tesoro\tesoro.exe
SET SOURCES=tesoro.cpp libinteractive\tesoro\tesoro_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -O2 -g -Wno-unused-result -Ilibinteractive/tesoro
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)

SET TARGET=libinteractive\tesoro\tesoro_debug.exe
SET SOURCES=tesoro.cpp libinteractive\tesoro\tesoro_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -g -Wno-unused-result -Ilibinteractive/tesoro
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)
