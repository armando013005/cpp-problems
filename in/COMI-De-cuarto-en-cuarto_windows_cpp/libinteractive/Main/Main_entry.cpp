...
static void __message_loop_AdivinaElNumero(int __current_function, int __noreturn) {
	...
	while (readfull(&__AdivinaElNumero_out, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x71651df7u: {
				// AdivinaElNumero -> Main.siguienteCuarto
				uint32_t __cookie;
				int siguienteCuarto_n;
				readfull(&__AdivinaElNumero_out, &siguienteCuarto_n, sizeof(int), 1);
				readfull(&__AdivinaElNumero_out, &__cookie, sizeof(__cookie), 1);

				int result =
				siguienteCuarto(siguienteCuarto_n);
			...
			case 0x685d93e7u: {
				// AdivinaElNumero -> Main.move
				uint32_t __cookie;
				int move_c;
				readfull(&__AdivinaElNumero_out, &move_c, sizeof(int), 1);
				readfull(&__AdivinaElNumero_out, &__cookie, sizeof(__cookie), 1);
...
void Player1(int x, int y) {
	...
	writefull(&__AdivinaElNumero_in, &__msgid, sizeof(__msgid));
	writefull(&__AdivinaElNumero_in, &x, sizeof(int));
	writefull(&__AdivinaElNumero_in, &y, sizeof(int));
	writefull(&__AdivinaElNumero_in, &__cookie, sizeof(__cookie));
	writeflush(&__AdivinaElNumero_in);
