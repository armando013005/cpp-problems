...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> jarras.programa
				uint32_t __cookie;
				int programa_objetivo;
				readfull(&__in, &programa_objetivo, sizeof(int), 1);
				int programa_capacidadJarra1;
				readfull(&__in, &programa_capacidadJarra1, sizeof(int), 1);
				int programa_capacidadJarra2;
				readfull(&__in, &programa_capacidadJarra2, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				programa(programa_objetivo, programa_capacidadJarra1, programa_capacidadJarra2);
...
void verterAgua(int fuente, int destino) {
	...
	writefull(&__out, &__msgid, sizeof(__msgid));
	writefull(&__out, &fuente, sizeof(int));
	writefull(&__out, &destino, sizeof(int));
	writefull(&__out, &__cookie, sizeof(__cookie));
	writeflush(&__out);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "\\\\.\\pipe\\libinteractive_jarras_jarras_in", O_RDONLY);
	openstream(&__out, "\\\\.\\pipe\\libinteractive_jarras_jarras_out", O_WRONLY);
