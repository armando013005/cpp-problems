...
static struct __stream __jarras_in, __jarras_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__jarras_in, "\\\\.\\pipe\\libinteractive_Main_jarras_in", O_WRONLY);
	openstream(&__jarras_out, "\\\\.\\pipe\\libinteractive_Main_jarras_out", O_RDONLY);
}
...
static void __message_loop_jarras(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__jarras_out, &__msgid, sizeof(__msgid), 0)) {
		if (__msgid == __current_function) return;
		switch (__msgid) {
			case 0x71651df7u: {
				// jarras -> Main.verterAgua
				uint32_t __cookie;
				int verterAgua_fuente;
				readfull(&__jarras_out, &verterAgua_fuente, sizeof(int), 1);
				int verterAgua_destino;
				readfull(&__jarras_out, &verterAgua_destino, sizeof(int), 1);
				readfull(&__jarras_out, &__cookie, sizeof(__cookie), 1);

				verterAgua(verterAgua_fuente, verterAgua_destino);

				writefull(&__jarras_in, &__msgid, sizeof(__msgid));
				writefull(&__jarras_in, &__cookie, sizeof(__cookie));
				writeflush(&__jarras_in);
				break;
			}
...
void programa(int objetivo, int capacidadJarra1, int capacidadJarra2) {
	...
	writefull(&__jarras_in, &__msgid, sizeof(__msgid));
	writefull(&__jarras_in, &objetivo, sizeof(int));
	writefull(&__jarras_in, &capacidadJarra1, sizeof(int));
	writefull(&__jarras_in, &capacidadJarra2, sizeof(int));
	writefull(&__jarras_in, &__cookie, sizeof(__cookie));
	writeflush(&__jarras_in);

	__message_loop_jarras(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__jarras_out, &__cookie_result, sizeof(__cookie_result), 1);
